...
def Make_EXON234_Panel(infile, outfile, BEAGLE2LINKAGE, plink):
    ...
    os.system(' '.join(["grep rs", infile + ".markers", '>', infile + ".STEP1_SNP.markers"]))

    os.system(' '.join(["grep \'HLA_[A-Z]_[0-9][0-9][0-9][0-9]\'", infile + ".markers", '>', infile + ".STEP1_class1_4dit.markers"]))

    os.system(' '.join(["grep \'HLA_[A-Z][A-Z][A-Z][0-9]_[0-9][0-9][0-9][0-9]\'", infile + ".markers", '>',
                        infile + ".STEP1_class2_4dit.markers"]))

    os.system(' '.join(["cat", infile + ".STEP1_SNP.markers", infile + ".STEP1_class1_4dit.markers",
                        infile + ".STEP1_class2_4dit.markers", '>', infile + ".STEP1_SNP_4dit.markers"]))
